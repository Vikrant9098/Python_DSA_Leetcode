class Solution(object):
    def maximumScore(self, grid):
        ...
        # If only one column, no score can be formed
        if n == 1:
            return 0

        # 3D DP array:
        # dp[i][curr_h][prev_h] represents max score till column i
        dp = [[[0] * (n + 1) for _ in range(n + 1)] for _ in range(n)]

        # Prefix max optimization arrays
        ...
        for i in range(1, n):
            for curr_h in range(n + 1):
                for prev_h in range(n + 1):
                    ...
                    # Case 1: current height <= previous height
                    if curr_h <= prev_h:
                    ...
                    # Case 2: current height > previous height
                    else:
                        extra_score = (
            ...
            # Update prefix max arrays for optimization
            for curr_h in range(n + 1):
